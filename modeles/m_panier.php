...
function ajout_prod(){
	...
	if (isset ($_SESSION['id_user']))
	{
		...
		if ((isset ($_POST['code_prod']) AND isset ($_POST['qte'])) OR (isset ($_SESSION['code_prod_temp']) AND isset ($_SESSION['qte_temp'])))
		{
			
			if (!$_SESSION['code_prod_temp']==0){$code_prod=$_SESSION['code_prod_temp'];}
				else{$code_prod=$_POST['code_prod'];}
			if (!$_SESSION['qte_temp']==0){$qte=$_SESSION['qte_temp'];}
				else{$qte=$_POST['qte'];}
			...
			if(!isset ($_SESSION['com'])){
				...
				$stat3=$db->prepare("SELECT qte_stock, remise FROM produit WHERE code_prod= :code_prod");
				$stat3->execute(array('code_prod' => $code_prod));
					$don = $stat3->fetch();
					
				/*Calcul de la nouvelle quantité de stock */
					$new_qte = $don['qte_stock']- $qte;
			...
			echo 'Produit ajouté au panier';
				...
				/* Récupération de la quantité en stock par code_prod */
				$stat3=$db->prepare("SELECT qte_stock, pu-remise*pu/100 AS pr FROM produit WHERE code_prod= :code_prod");
				$stat3->execute(array('code_prod' => $code_prod));
					$doni = $stat3->fetch();
...
function calcul_panier(){
	...
	$stat2=$db->prepare("SELECT SUM(pu*qte) AS sstotal FROM ligne_commande JOIN produit 
